    ...
    def _build_forward_scalars(self):
        scalars = torch.tensor(
            [
                ...
                float(self.d_s2),
                float(self.C),
                float(_require_config_attr("LIF_DECAY")),
                float(_require_config_attr("LIF_THRESHOLD")),
                float(_require_config_attr("HALT_THRESHOLD")),
                float(self.lambda_sparsity),
                float(self.trace_gain),
                float(_require_config_attr("PARAM_COST_SCALE")),
                float(_require_config_attr("PHASE_0_KEEP_RATIO")),
                float(self.current_phase),
