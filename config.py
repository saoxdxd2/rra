...
class Config:
    ...
    D = _CONFIG_DATA.get('model', {}).get('D', 256)
    D_S1 = _CONFIG_DATA.get('model', {}).get('D_S1', 64)
    D_S2 = _CONFIG_DATA.get('model', {}).get('D_S2', 512) 
    ...
    MYELIN_COST = 0.001
    EPISODIC_CAPACITY = _CONFIG_DATA.get('model', {}).get('episodic_capacity', 1000)
    EPISODIC_READ_TOP_K = int(_CONFIG_DATA.get('model', {}).get('episodic_read_top_k', 3))
    EPISODIC_MAX_SCAN = int(_CONFIG_DATA.get('model', {}).get('episodic_max_scan', 512))
    EPISODIC_READ_EVERY = int(_CONFIG_DATA.get('model', {}).get('episodic_read_every', 2))
    EPISODIC_DENSE_MODE = bool(_CONFIG_DATA.get('model', {}).get('episodic_dense_mode', False))
    EPISODIC_KEY_LATENT_DIM = int(_CONFIG_DATA.get('model', {}).get('episodic_key_latent_dim', 64))
    EPISODIC_VALUE_LATENT_DIM = int(_CONFIG_DATA.get('model', {}).get('episodic_value_latent_dim', 64))
    EPISODIC_DENSE_CAPACITY_MULTIPLIER = float(_CONFIG_DATA.get('model', {}).get('episodic_dense_capacity_multiplier', 1.0))
    EPISODIC_HYBRID_MODE = bool(_CONFIG_DATA.get('model', {}).get('episodic_hybrid_mode', False))
    EPISODIC_HOT_CAPACITY = int(_CONFIG_DATA.get('model', {}).get('episodic_hot_capacity', 256))
    EPISODIC_HOT_TOP_K = int(_CONFIG_DATA.get('model', {}).get('episodic_hot_top_k', 2))
    EPISODIC_COLD_TOP_K = int(_CONFIG_DATA.get('model', {}).get('episodic_cold_top_k', 2))
    EPISODIC_HYBRID_FALLBACK_THRESHOLD = float(_CONFIG_DATA.get('model', {}).get('episodic_hybrid_fallback_threshold', 0.35))
