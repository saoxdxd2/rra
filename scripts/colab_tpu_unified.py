...
try:
    ...
    is_tpu = True
    print("PyTorch Unified Cognitive Engine running on TPU (XLA)")
except ImportError:
...
# ==============================================================================
# 3. THE UNIFIED SPATIO-TEMPORAL ENGINE
# ==============================================================================
class UnifiedCognitiveEngine(nn.Module):
    def __init__(self, num_neurons, max_delay):
        ...
        # -----------------------------------------------------
        # 1. Spatio-Temporal Component (Axonal Delays)
        # -----------------------------------------------------
        self.delay_logits = nn.Parameter(torch.randn(num_neurons, num_neurons, max_delay))
        
        # -----------------------------------------------------
        # 2. Reinforcement Learning Component (Dopamine Traces)
        # -----------------------------------------------------
        ...
        # We start with random synapses. The traces + delays must work together to tune them!
    ...
    def forward(self, input_spikes, reward_signal, sim_steps):
        ...
        # Softmax over delay taps
        delay_probs = F.softmax(self.delay_logits, dim=-1)
        
        # Initialize local synaptic weights state from the base parameters
        synaptic_weights_state = self.synaptic_weights.clone()
        ...
        for t in range(sim_steps):
            ...
            # --- POLYCHRONIZATION (Gather Delayed Spikes) ---
            arriving_spikes = torch.einsum('bdi,ijd->bij', spike_history, delay_probs)
            current_in = torch.sum(arriving_spikes * synaptic_weights_state.unsqueeze(0), dim=1)
            ...
            # --- DOPAMINE CREDIT ASSIGNMENT ---
            current_reward = reward_signal[:, t] * self.dopamine_sensitivity
            
            # The Universal Plasticity MLP bridges the spatio-temporal gap!
            ...
            reward_expanded = current_reward.view(batch_size, 1, 1).expand(-1, self.num_neurons, self.num_neurons)
            
            delta_w = self.plasticity_mlp(v_pre_expanded, v_post_expanded, ca_expanded, reward_expanded)
            
            # The synapses physically update in real-time using local state (mean across batch)
            synaptic_weights_state = synaptic_weights_state + delta_w.mean(dim=0) * 0.001
            
        return torch.stack(output_spikes, dim=1)

# ==============================================================================
# 4. THE GRAND UNIFICATION TASK
# ==============================================================================
def run_unified_bptt():
    NUM_NEURONS = 10
    SIM_STEPS = 300 
    MAX_DELAY = 15
    BATCH_SIZE = 1 
    
    print(f"Initializing Unified Brain Engine.")
    print(f"Neurons: {NUM_NEURONS}, Max Delay: {MAX_DELAY}ms, Sim Window: {SIM_STEPS}ms")
    
    model = UnifiedCognitiveEngine(num_neurons=NUM_NEURONS, max_delay=MAX_DELAY).to(device)
    
    # We optimize BOTH the physical routing (delays) AND the molecular memory (traces)
    ...
    # ---------------------------------------------------------
    # THE ULTIMATE SENSORIMOTOR TASK
    # ---------------------------------------------------------
    # 1. Sensory Phase
    inputs = torch.zeros(BATCH_SIZE, SIM_STEPS, NUM_NEURONS, device=device)
    inputs[:, 10, 0] = 1.0  # Sensor 0
    inputs[:, 20, 1] = 1.0  # Sensor 1
    
    # 2. Action Phase (Must perfectly polychronize by T=40 to hit Output 9)
    target_spikes = torch.zeros(BATCH_SIZE, SIM_STEPS, device=device)
    target_spikes[:, 40] = 1.0
    
    # 3. Reward Phase (Dopamine arrives at T=150)
    reward_signal = torch.zeros(BATCH_SIZE, SIM_STEPS, device=device)
    reward_signal[:, 150] = 1.0 
    ...
    for epoch in range(1, 1001):
        ...
        # Reset synapses to random baseline to force the traces and delays to do the heavy lifting
        ...
        # Loss: Polychronization accuracy
        ...
        print(f"Epoch {epoch:03d} | Err: {loss_val:.5f} | Traces(Pre:{t_pre:.1f} Post:{t_post:.1f} Ca:{t_ca:.1f})")
        ...
        if loss_val < 0.005:
            ...
            print("Axonal Delays and Dopamine Traces are perfectly aligned.")
            print("============================================================")
            break
